...
export async function createBusinessProfile(req, res) {
    try {
        ...
        if (!userId) {
            return res.status(401).json({ success: false, menubar: 'Unauthorized' });
        }
        ...
        return res.status(500).json({ success: false, message: 'Server error while creating business profile' });
    }
}
...
export async function updateBusinessProfile(req, res) {
    try {
        ...
        return res.status(500).json({ success: false, message: 'Server error while updating business profile' });
...
export async function getMyBusinessProfile(req, res) {
    try {
        ...
        return res.status(500).json({ success: false, message: 'Server error while fetching business profile' });
    }
}